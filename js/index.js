...
const Game = {
	...
	startGame: function () {
		...
		Game.elements.previewBall = Game.generateballBody(Game.width / 2, previewBallHeight, 0, { isStatic: true });
		...
		Events.on(engine, 'collisionStart', function (e) {
			for (let i = 0; i < e.pairs.length; i++) {
				...
				if (bodyA.circleRadius >= Game.ballSizes[Game.ballSizes.length - 1].radius) {
					newSize = 0;
				}
				...
				Composite.add(engine.world, Game.generateballBody(midPosX, midPosY, newSize));
	...
	addball: function (x) {
		...
		const latestball = Game.generateballBody(x, previewBallHeight, Game.currentballSize);
		...
		Game.elements.previewBall = Game.generateballBody(render.mouse.position.x, previewBallHeight, Game.currentballSize, {
			isStatic: true,
			collisionFilter: { mask: 0x0040 }
		});
